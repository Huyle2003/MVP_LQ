...
import { createSkinButtonFromCropped } from '../../services/skinButtonService.js'

export default function CreateButtonFromCropModal({ open, croppedItem, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!name.trim()) {
      setError('Vui lòng nhập tên nút bấm')
      return
    }
    ...
    try {
      setSaving(true)
      setError('')
      await createSkinButtonFromCropped({
        skin_id: skinId,
        name: name.trim(),
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box modal-wide" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <h3>Thêm nút bấm từ ảnh đã cắt</h3>
          <button className="modal-close" onClick={onClose}><X size={20} /></button>
        </div>

        {croppedItem && (
          <div className="crop-modal-preview">
            <img src={croppedItem.image_url} alt="Nút bấm đã cắt" />
          </div>
        )}
