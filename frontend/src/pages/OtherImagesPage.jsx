...
import {
  getOtherImages, createOtherImage, updateOtherImage, deleteOtherImage,
} from '../services/otherImageService.js'
...
function FormModal({ open, editItem, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (editItem) await updateOtherImage(editItem.id, fd)
      else await createOtherImage(fd)
      onSaved()
...
/* ─── Main Page ──────────────────────── */
export default function OtherImagesPage() {
  ...
  async function loadItems() {
    try {
      setLoading(true)
      const data = await getOtherImages({ keyword: keyword || undefined, status: statusFilter || undefined })
      setItems(data)
  ...
  function handleDelete(item) {
    setConfirm({
      ...
      async onConfirm() {
        try {
          await deleteOtherImage(item.id)
          setConfirm(null)
          loadItems()
  ...
  return (
    <div className="page-section">
      ...
      <div className="page-header">
        <div>
          <h2>Danh mục ảnh khác</h2>
          <p>Quản lý các ảnh phụ trợ khác của hệ thống.</p>
        </div>
