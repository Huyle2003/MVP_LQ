...
import {
  getSkinKillNotifications, createSkinKillNotification, updateSkinKillNotification, deleteSkinKillNotification,
} from '../services/skinKillNotificationService.js'
...
function FormModal({ open, editItem, heroes, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (editItem) await updateSkinKillNotification(editItem.id, fd)
      else await createSkinKillNotification(fd)
      onSaved()
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box modal-wide" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h3>{editItem ? 'Sửa thông báo hạ' : 'Thêm thông báo hạ'}</h3>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="field"><span>Tên thông báo *</span>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Tên thông báo hạ" /></div>
...
export default function NotificationTemplatePage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      setItems(await getSkinKillNotifications({
  ...
  function handleDelete(item) {
    setConfirm({ open: true, title: 'Xoá thông báo hạ', msg: `Xoá "${item.name}"?`,
      onConfirm: async () => {
        try { await deleteSkinKillNotification(item.id); loadData() }
  ...
  return (
    <div className="page-section">
      <div className="page-header">
        <div>
          <h2>Danh mục thông báo hạ</h2>
          <p>Quản lý ảnh thông báo hạ liên kết theo từng skin.</p>
        </div>
        <button className="secondary-button" onClick={handleAdd}><Plus size={16} /> Thêm thông báo</button>
      </div>
      ...
      <div className="table-card">
        ...
        : items.length === 0 ? <div className="empty-state"><FileImage size={40} /><h3>Chưa có thông báo hạ nào</h3></div>
        : <table><thead><tr>
